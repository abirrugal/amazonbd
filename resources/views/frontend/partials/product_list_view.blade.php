             ...
             <div class="col-8 ">
               <div class="card-body">
                   ...
                   <p class="price-title-home text-start mb-3">
                       ...
                       @if($product->sale_price !== null && $product->sale_price > 0) Price (BDT) :<strike> {{$product->price}}</strike> <br> BDT {{number_format($product->sale_price,2) }} @else Price (BDT) : {{number_format($product->price,2)}}
                       @endif
                   </p>                              
...
{{-- Large Screen --}}
       ...
       @if($products) @foreach ($products as $product)
           ...
           <div class=" shadow-sm h-100 mb-3">
               ...
               <div class="card-body">
                   <p class="card-text text-start product-title "> <a class=" text-decoration-none" href="{{route('frontend.product.show',$product->slug)}}"> {{ Illuminate\Support\Str::limit($product->title, 40) }}</a> </p>
